...
const getDestructuredHUDParam = (value) => {
  // Destructure array shrothand
  if (value && Array.isArray(value)) {
    const args = value;
    let valueOpt = {};
    if (args.length > 0) valueOpt.value = args[0];
    if (args.length > 1) valueOpt.min = args[1];
    if (args.length > 2) valueOpt.max = args[2];
    if (args.length > 3) valueOpt.step = args[3];
    if (args.length > 4) valueOpt.name = args[4];
    value = valueOpt;
  }
  return value;
};
...
class SketchManager {
  ...
  _getParams () {
    ...
    return Object.keys(this._params).reduce((dict, key) => {
  ...
  update (opt = {}) {
    ...
    // Modify params
    if ('params' in opt) {
      // Smartly merge in each param
      if (this._settings.params) {
        Object.keys(this._settings.params).forEach(key => {
          let value = this._settings.params[key];

          value = getDestructuredHUDParam(value);

          if (value == null) {
            // User wishes to remove a param
            this._params[key] = value;
          } else if (typeof value === 'object') {
            // Incoming value has options
            if (typeof this._params[key] === 'object') {
              // Merge in new data like min/max etc
              Object.assign(this._params[key], value);
            } else {
              // Current value doesn't have options yet, set them up
              this._params[key] = value;
            }
          } else {
            // Incoming value is a literal value
            if (typeof this._params[key] === 'object') {
              // Already have min/max etc settings, just replace value
              this._params[key].value = value;
            } else {
              // No min/max settings, just assign new value
              this._params[key] = value;
            }
          }
        });
      ...
      this._props.params = this._getParams();

      // If new update includes params, try to re-attach if necessary
      if (this._settings.params) {
        HUD.attach(this);
      } else {
        HUD.detach(this);
  ...
  mount () {
    ...
    // If user requested params, attach a HUD
    if (this._settings.params) {
      HUD.attach(this);
    }
  }
