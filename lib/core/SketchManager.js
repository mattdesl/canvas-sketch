...
import { rightNow, isBrowser, defined, isWebGLContext, isCanvas, getClientAPI } from '../util';
...
class SketchManager {
  ...
  record () {
    ...
    streamStart(exportOpts)
      .then(response => {
        // If streaming works, we start animation
        this._raf = window.requestAnimationFrame(tick);
      })
      .catch(err => {
        // Otherwise we end
        console.error('Error starting media stream - recording stopped');
        console.error(err);
        this.endRecord();
        this.stop();
        this.run();
