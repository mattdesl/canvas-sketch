...
import { getPropValue } from './propUtil';
...
const compare = (a, b) => {
  for (let key in a) {
    ...
    if (!compareItem(a[key], b[key])) return false;
  }
  for (let key in b) {
    if (key === 'value') continue;
    if (!compareItem(a[key], b[key])) return false;
...
export default class ParamManager {
  ...
  addProp (key, opt = {}) {
    ...
    if (this.storage != null && key in this.storage) {
      ...
      if (compare(opt, this.storage[key])) {
  ...
  updatePropValue (key, value, computedProps) {
    ...
    if (reservedKeys.includes(key)) {
      ...
      this.proxy[key] = getPropValue(curProp.value, curProp);
    }
  }
