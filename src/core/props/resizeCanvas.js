...
const DEFAULT_CANVAS_SIZE = [300, 150];
...
function getParentSize(props, settings) {
  ...
  if (curWindow == null || curWindow.document == null) {
    // node.js environment with no window defined
    return DEFAULT_CANVAS_SIZE.slice();
  }

  if (curWindow.innerWidth == null || curWindow.innerHeight == null) {
    // window is defined but no inner width / height
    // also assume node.js environment
    return DEFAULT_CANVAS_SIZE.slice();
  }

  let element = settings.parent || curWindow;

  if (
    element === curWindow ||
    element === curWindow.document ||
    element === curWindow.document.body
  ) {
    return [curWindow.innerWidth, curWindow.innerHeight];
  } else {
...
export default function resizeCanvas(props, settings) {
  ...
  const pixelsPerInch =
    typeof settings.pixelsPerInch === "number" &&
    isFinite(settings.pixelsPerInch)
      ? settings.pixelsPerInch
      : 72;

  const devicePixelRatio = defined(
    curWindow ? curWindow.devicePixelRatio : undefined,
    1
  );
  ...
  if (
    typeof settings.pixelRatio === "number" &&
    isFinite(settings.pixelRatio)
  ) {
  ...
  if (
    ...
    isFinite(settings.maxPixelRatio)
  ) {
  ...
  canvasWidth = scaleToView
    ? Math.round(pixelRatio * styleWidth)
    : Math.round(pixelRatio * realWidth);
  canvasHeight = scaleToView
    ? Math.round(pixelRatio * styleHeight)
    : Math.round(pixelRatio * realHeight);

  const viewportWidth = scaleToView
    ? Math.round(styleWidth)
    : Math.round(realWidth);
  const viewportHeight = scaleToView
